...
const NavButton = ({ href, isExternal, className, onClick, children }) => {
    if (isExternal) {
...
export default function Header() {
    ...
    const isStaff  = user?.role && ['admin', 'driver', 'cleaner'].includes(user.role.name);
    const dashboardUrl = isStaff ? '/admin' : '/dashboard';
            ...
            <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8">
                    ...
                    <div className="flex items-center gap-2 sm:gap-3">
                        ...
                        {user ? (
                            ...
                            >
                                ...
                                <span className="hidden md:inline">{t('navigation.dashboard')}</span>
                            </NavButton>
                        ) : (
                            <Link
                                href="/login"
                                className="hidden sm:flex items-center gap-2 bg-purple-600 hover:bg-purple-700 active:bg-purple-800 text-white text-sm font-semibold px-4 py-2 rounded-lg transition-all duration-200 shadow-sm hover:shadow-md whitespace-nowrap group"
                            >
                                <PersonIcon className="w-4 h-4 transition-transform duration-200 group-hover:scale-110" />
                ...
                >
                    <nav className="py-4 space-y-1 border-t border-gray-200">
                        ...
                        {/* ── Login / Dashboard CTA inside mobile menu ── */}
                        <div className="px-2 pt-1">
                            <NavButton
                                href={user ? dashboardUrl : '/login'}
                                isExternal={!!user && isStaff}
                            ...
                            >
                                ...
                                {user ? t('navigation.dashboard') : t('navigation.login')}
                            </NavButton>
                        </div>
                    </nav>
                </div>
