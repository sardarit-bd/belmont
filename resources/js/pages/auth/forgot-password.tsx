...
function ForgotPasswordInner({ status }: { status?: string }) {
    ...
    // Map the PHP status key to a translated string and a visual style.
    // The controller always passes a key like 'passwords.sent' or 'passwords.throttled'.
    const statusConfig: Record<string, { i18nKey: string; style: 'success' | 'warning' }> = {
        'passwords.sent':      { i18nKey: 'auth.status_reset_sent',      style: 'success' },
        'passwords.throttled': { i18nKey: 'auth.status_reset_throttled', style: 'warning' },
    };

    const resolvedStatus = status ? statusConfig[status] ?? { i18nKey: 'auth.status_reset_sent', style: 'success' } : null;
            ...
            <div className="min-h-screen grid grid-cols-1 lg:grid-cols-2 bg-[#faf9f7]">
                ...
                <div className="flex items-center justify-center p-8 lg:p-12">
                    <div className="w-full max-w-[400px]">
                        ...
                        {/* Status banner — translated and colour-coded */}
                        {resolvedStatus && (
                            <div className={`flex items-center gap-2 border rounded-xl px-4 py-3 mb-6 text-[13px] ${statusStyles[resolvedStatus.style]}`}>
                                {statusIcons[resolvedStatus.style]}
                                {t(resolvedStatus.i18nKey)}
                            </div>
                        )}
